    ...
    while not exit:
        ...
        try:
            ...
            if len(list1_sd) == len(list2_sd): #check if two files have the same numbre of sentences
                (oraciones, errorTotal, numbers1, numbers2) = compare(mode, map, list_sen, list1_sd, list2_sd)
                result(oraciones, errorTotal, numbers1, numbers2)
...
def compare(mode, map, list_sen, list1_sd, list2_sd):
    ...
    for element in list1_sd: #compute the counters of each category
        ...
        if mode == 1: #all sentences
            ...
            blanco=" "
            print(element + blanco*spaces + list2_sd[counter] + blanco*spaces2 + list_sen[counter])     
            counter+=1 
